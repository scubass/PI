...
{
	int line, column, media = 0;

	if (w >= 3 && w % 2 != 0) {
		for (int j = 0; j < ALTO; j++) {
			for (int h = 0; h < ANCHO; h++) {
				// line = normalize_line(j);
				// column = normalize_line(h);
				line = j - j % w;
				column = h - h % w;
				for (int m = line; m < (line + w); m++) {
					for (int i = column; i < (column + w); i++) {
						// media += imagen[m][i];
						printf("%d  | ", imagen[m][i]);
					}
					printf("\n\n\n");
				}
				// printf("suma de la sub marix: %d \n", media);
				// printf("media / w x w:  %d\n\n", media / (w * w));
				// imagen[j][h] = media / (w * w);
				// media = 0;
			}
		}	
	}
...
normalize_line(int number) 
{
	if (number == 0) {
		return 0;
	} else if (number == ANCHO - 1 || number == ALTO - 1) {
		return number - 2;
	}
	return number - 1;
}
